...
function loadXMLDoc() {
    var userslist = {"张三":[["张三"],"1234"],"李四":[["李四"],"5678"],"王五":[["张三","王五"],"6666"],"boss":[["张三","李四","王五","boss"],"8888"]};  //字典结构{用户名：【【权限】，密码】}
    var getuser = document.getElementById("username").value;
    var getpw = document.getElementById("pw").value;
    if(getuser!=""&&getpw!=""&&getpw===userslist[getuser][1]){  //验证账号
        ...
        for (var i = 1; i < datas.length; i++) { //遍历请求的每行内容,
            if(userslist[getuser][0].indexOf(datas[datas.length-i][5])!=-1){  //验证账号权限，因为是从下往上验证，所以datas的顺序要倒着写
    ...
    document.getElementById("countbar").innerText="一共搜索出"+count.toString()+"条结果";

    }else{alert("账号或密码错误")};
}
window.onload=function(){
var submitbtn = document.getElementById("submit");
submitbtn.addEventListener("click",loadXMLDoc);
}
